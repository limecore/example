...
LimecoreApplication(Path("."), None).using(
    ConfigurationModule("example", enable_commandline_arguments=False)
).using(LoggingModule()).using(RabbitMQModule()).using(app_factory).run(App)
